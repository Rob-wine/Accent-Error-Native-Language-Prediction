...
def eval_network(weights_file="weights.hdf5", classpath="Preproc/Test/", batch_size=40):
    ...
    labels =[]
    for i in range(Y_test.shape[0]):
         labels.append(class_names[decode_class(Y_test[i],class_names)])
    pd.Dataframe({'ground_truth_test':labels}).to_json('test_labels.json')


    print("Counting mistakes ")
    count_mistakes(y_scores,Y_test,paths_test,class_names)
    df_preds = pd.DataFrame(y_scores)
    df_preds.columns = class_names
    top_preds = df_preds.idxmax(1)
    top_preds_df = pd.DataFrame(top_preds)
    top_preds_df.to_json('predicted_classes.json')
